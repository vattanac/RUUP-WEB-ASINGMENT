...
xmlhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
        ...
        var items = "";
        addCatNameTitle(myObj.lorkta[1].culture);
        for (var a in myObj.lorkta[1].culture) {
            items += mascard(myObj.lorkta[1].culture[a]);
        }
...
function getCardID(id) {
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (var a in myObj.lorkta[3].entertain) {
                //console.log( myObj.lorkta[0].art[a])
                items += mascard(myObj.lorkta[3].entertain[a]);
                if (id == myObj.lorkta[3].entertain[a].id) {
...
function addCatNameTitle(obj) {
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            var myObj = JSON.parse(this.responseText);
            var item = myObj.lorkta[3].entertain;
